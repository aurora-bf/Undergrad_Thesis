...
figure(1)
loglog(ep,numpres,'-')
title('\epsilon versus number points preserved')
ylabel('Number of collocation points present')
xlabel('\epsilon')
hold on;
grad=polyfit(log(ep),log(numpres),1)
slope(b)=grad(:,1)
 
end
 loglog(ep,ep.^(-0.5),'--k')
 legend('tanh(x)','gaussmf(x)','Reference line with slope -0.5')
